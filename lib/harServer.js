        ...
        req.on('end', function () {
            ...
            var harEntry = _.find(har.log.entries, function (entry) {
                ...
                if (reqBody ||
                    ...
                    var multipartHarHeader = entry.request.headers.find(function (header) {
                        return header.name.toLowerCase() === 'content-type' &&
                            header.value.indexOf('multipart/form-data; boundary=') === 0;
                    });

                    var multipartRequestHeader = (req.headers['content-type'] &&
                            req.headers['content-type'].indexOf('multipart/form-data; boundary=') === 0) ?
                        req.headers['content-type'] : undefined;

                    if (multipartRequestHeader && multipartHarHeader) {
                        var requestBoundaryDelimeter;
                        var harBoundaryDelimeter;

                        var boundaryDelimeter = Math.random().toString(36).substr(2, 9);

                        requestBoundaryDelimeter = multipartRequestHeader.substr(30);
                        harBoundaryDelimeter = multipartHarHeader.value.substr(30);

                        reqBody = reqBody.replace(new RegExp(requestBoundaryDelimeter, 'g'), boundaryDelimeter);
                        entry.request.postData.text = entry.request.postData.text.replace(
                            new RegExp(harBoundaryDelimeter, 'g'), boundaryDelimeter);
                    }
