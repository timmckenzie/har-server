...
const cycle = function * (iteration) {
    const buffer = [];
    for(const x of iteration) {
        buffer.push(x);
        yield x;
    }
    while(true) {
...
const entryFinder = (rawEntries, config) => {

    const makeKey = (url, method, data) => {
        const parsedUrl = urlLib.parse(url);
        return JSON.stringify({
            method,
            data,
            path: parsedUrl.path,
            host: config.dontMatchOnHost ? null : parsedUrl.host
        });
    };

    const groupedEntries = _.groupBy(rawEntries, ({request: {url, method, postData}}) => makeKey(url, method, postData?.text));
    const nextResultLookup = Object.fromEntries(Object.entries(groupedEntries).map(([k, lst]) => [k, cycle(lst)[Symbol.iterator]()]));

    return (req, reqBody) => nextResultLookup[makeKey(req.url, req.method, reqBody)]?.next().value;
}
 ...
 * @param config.dontMatchOnHost  {boolean} Do not match on host when matching against har entries
 * @returns {HarServer}
 */
function HarServer(config) {
    ...
    this.readHar = function readHar(harFileName) {
        ...
        return fs.accessAsync(self.config.harFileName, fs.R_OK).catch(function (err) {
            ...
            // Strip from the log entries where the connection failed which will cause issue in validation
            har.log.entries = har.log.entries.filter(x => x.response.status);
    ...
    this._requestHandler = function _requestHandler(req, res) {
        ...
        const findHarEntryForRequest = entryFinder(har.log.entries, config);
        ...
        req.on('end', function () {
            ...
            if (reqSize > 0) {
                reqBody = Buffer.concat(reqChunks, reqSize).toString();
            }

            var harEntry = findHarEntryForRequest(req, reqBody);
